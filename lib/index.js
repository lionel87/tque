...
function createInterface(data, que = [], base) {
    ...
    self.consume = async () => {
        ...
        while (self.que.length > 0) {
            let results = await self.que.shift().call(self.api, self.data);
            ...
            if (results.length > 0) {
                    ...
                    if (result !== false) { // if not exiting
                        if (result !== true) {
                            if (result && typeof result === 'object') { // object
                                if (result[detachObjectSymbol]) { // do not merge
                                    delete result[detachObjectSymbol];
                                    self.branch(result);
                                }
                                else { // please merge
                ...
                if (result0 && typeof result0 === 'object') { // que continues ...
                    if (result0[detachObjectSymbol]) { // do not merge
                        delete result0[detachObjectSymbol];
                        self.data = result0;
                    }
                    else { // please merge
            ...
            for (let handlerResult of handlerResults) {
                ...
                if (handerResult0 !== true) {
                    if (handerResult0 && typeof handerResult0 === 'object') { // object
                        if (handerResult0[detachObjectSymbol]) { // do not merge
                            internals.data = handerResult0;
                        }
                        else { // please merge
                            assignDeep(internals.data, handerResult0);
                        }
                    }
                }
                else {
